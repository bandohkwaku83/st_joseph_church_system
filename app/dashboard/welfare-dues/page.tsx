...
export default function WelfareDuesPage() {
  ...
  const [payments, setPayments] = useState<WelfarePayment[]>([
    {
      ...
      month: CURRENT_MONTH,
      amount: 50,
      paymentDate: `${CURRENT_YEAR}-${String(CURRENT_MONTH).padStart(2, '0')}-03`,
      method: 'Cash',
    },
    {
      id: '2',
      ...
      month: CURRENT_MONTH,
      amount: 50,
      paymentDate: `${CURRENT_YEAR}-${String(CURRENT_MONTH).padStart(2, '0')}-04`,
      method: 'Mobile Money',
    },
    {
      id: '3',
      memberId: 4,
      memberName: 'Akosua Adjei',
      year: CURRENT_YEAR,
      month: CURRENT_MONTH,
      amount: 25,
      paymentDate: `${CURRENT_YEAR}-${String(CURRENT_MONTH).padStart(2, '0')}-06`,
  ...
  const handleRecordPayment = (values: {
    ...
    paymentDate: string;
    amount: number;
    method: WelfarePayment['method'];
  }) => {
    const member = members.find((m) => m.id === values.memberId);
    if (!member) return;

    const newPayment: WelfarePayment = {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        ...
        <Button onClick={() => setShowPaymentDrawer(true)} className="sm:w-auto w-full">
